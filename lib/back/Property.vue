<template>
  ...
  <div v-else-if="fullSchema.type === 'string' && ['date', 'date-time'].includes(fullSchema.format)">
    ...
    <v-select v-else-if="(fromUrl || fullSchema['x-fromData'] || fullSchema['x-getData']) && fullSchema.description !== 'custom'"
              :items="selectItems"
              v-model="modelWrapper[modelKey]"
              :name="fullKey"
              :label="label"
  ...
  <div v-else-if="(fullSchema.description === 'custom') && (fromUrl || fullSchema['x-fromData'] || fullSchema['x-getData'])">
  ...
  methods: {
    ...
    updateSelectItems() {
      ...
      if (!this.rawSelectItems) {
        ...
        (this.fullSchema.description !== 'custom' && (this.fullSchema['x-type'] === 'object' || this.fullSchema.type === 'object') && this.fullSchema.properties && Object.keys(this.fullSchema.properties).length) ||
